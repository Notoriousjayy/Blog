...
/*
 * New Perspectives on HTML5, CSS3, and JavaScript 6th Edition Tutorial 11
 * Tutorial Case
 * 
 * Author: Date:
 * 
 * Global Variables ================
 * 
 * puzzleCells References the TD cells within the Hanjie table grid.
 * 
 * cellBackground Stores the current background color of the puzzle cells during
 * the mouseover event.
 * 
 * 
 * Function List =============
 * 
 * init() Run when the web page is loaded; displays puzzle 1 and loads the event
 * handlers for the web page buttons.
 * 
 * setupPuzzle() Sets up a new puzzle, adding the event handlers for every
 * puzzle cell.
 * 
 * swapPuzzle(e) Swaps one puzzle for another based on the button being clicked
 * by the user. Confirms the change before swapping in the new puzzle.
 * 
 * setBackground(e) Sets the background color of the puzzle cells during the
 * mousedown event
 * 
 * extendBackground(e) Extends the background color of the original puzzle cell
 * during the mouseenter event.
 * 
 * endBackground() Ends the action of extending the cell backgrounds in response
 * to the mouseup event.
 * 
 * drawPuzzle(hint, rating, puzzle) Returns a text string of the HTML code to
 * display a hanjie Web table based on the contents of multi-dimensional array,
 * puzzle.
 * 
 */
...
function init() {
	...
	document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle1Hint,
			puzzle1Rating, puzzle1);
...
function swapPuzzle(e) {
	...
	case "puzzle1":
		...
		break;
	case "puzzle2":
		document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle2Hint,
				puzzle2Rating, puzzle2);
		break;
	case "puzzle3":
		document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle3Hint,
				puzzle3Rating, puzzle3);
		break;
	}
...
function setupPuzzle() {
	...
	/* Matches all of the data cells in the puzzle */
	...
	for (var i = 0; i < puzzleCells.length; i++) {
		...
		puzzleCells[i].style.cursor = "url(./images/pencil.png), pointer";
	}
	...
	document
			...
			.addEventListener(
					...
					function() {
						...
						/* Dislpay incorrect white cells in pink */
...
function setBackground(e) {
	...
	if (e.shiftKey) {
		...
		cursorType = "url(./images/eraser.png), cell";
	} else if (e.altKey) {
		cellBackground = "rgb(255, 255, 255)";
		cursorType = "url(./images/cross.png), crosshair";
	} else {
		cellBackground = "rgb(101, 101, 101)";
		cursorType = "url(./images/pencil.png), pointer";
	}
	e.target.style.backgroundColor = cellBackground;
...
function drawPuzzle(hint, rating, puzzle) {
	...
	/*
	 * puzzle is a multidimensional array containing the Hanjie puzzle layout.
	 * Marked cells are indicated by the # character. Empty cells are indicated
	 * by an empty text string. First, determine the number of rows and columns
	 * in the puzzle
	 */
	...
	/*
	 * Loop through the rows to create the rowCount array containing the totals
	...
	/*
	 * Loop through the columns to create the colCount array containing the
	 * totals for each column in the puzzle
	 */
	...
	/*
	 * Create a Web table with the id, hanjieGrid, containing headers with the
	 * row and column totals. Each marked cell has the class name, marked; each
